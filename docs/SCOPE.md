...
- **Governance not yet in CI** (audit #12): detekt / ktlint, CodeQL, SBOM, coverage thresholds, reproducible builds and commit signing. Each adds dependencies or maintainer-side keys; tracked, not started. What is in place: every action is pinned by SHA, every Gradle artifact is checksum-verified, and Dependabot alerts are on — automatic security-update PRs are deliberately off, because a bot cannot regenerate the verification metadata a Gradle bump needs.
