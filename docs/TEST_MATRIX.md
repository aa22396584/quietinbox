...
| Instrumented Compose semantics | What a screen reader actually reads for a message row | `MessageBubbleSemanticsTest` (2: in a group chat exactly one node in the merged tree carries both the sender and the body — the negative control asserts they are two `Text`s in the unmerged tree, so the match is the merge working and not one composable) | `ANDROID_SERIAL=<emulator> ./gradlew :feature:conversation:connectedDebugAndroidTest` |
