...
| L1 JVM replay | Pure Kotlin parser / identity / reconcile / analytics | 79 tests in `core:*` (model 5, parser 13, identity 5, reconcile 22 incl. `ReconcilerIdAlignmentTest`, analytics 34), 45 in `parsers:apps`, 5 in `app` (reminders incl. `ReminderPolicy`); two 1,000-iteration property tests: distinct content must be accepted exactly once (seed 20260905), repeated id-less content must never duplicate and replays must never shrink the window (seed 20260906) | `./gradlew :core:model:test :core:parser:test :core:identity:test :core:reconcile:test :core:analytics:test :parsers:apps:test` |
