...
| L1 JVM replay | Pure Kotlin parser / identity / reconcile / analytics | 79 tests in `core:*` (model 5, parser 13, identity 5, reconcile 22 incl. `ReconcilerIdAlignmentTest`, analytics 34), 43 in `parsers:apps`, 5 in `app` (reminders incl. `ReminderPolicy`); two 1,000-iteration property tests: distinct content must be accepted exactly once (seed 20260905), repeated id-less content must never duplicate and replays must never shrink the window (seed 20260906) | `./gradlew :core:model:test :core:parser:test :core:identity:test :core:reconcile:test :core:analytics:test :parsers:apps:test` |
...
| Instrumented storage | Real SQLCipher + Room migrations | `VaultRoundTripTest` (journal → commit → search → suppression → reopen with persisted key; deleted conversation does not resurrect on replay), `MigrationTest` (1→2, 2→3 and 3→4 against exported schemas; 3→4 asserts both new columns come back null on rows written before them, and that a gap may then be inserted with a source), `DemoDataTest` (debug demo seed → counts and projection → idempotent re-seed → clear leaves nothing; the seed is localised through `DemoLocalisation` for zh-Hans / ja / ko app languages), `DeletionGraphTest` (5: journal payload cleared on leaving PENDING; deleting the newest message rebuilds the projection; expired copies hidden before retention and projection rebuilt after it; removing a source with data leaves nothing behind; delete-everything verified and no cached cipher outlives the old key), `SearchPagingTest` (2: 250 false-positive candidates neither hide a true hit nor under-fill a page, pages resume by cursor without overlap; a deletion token suppresses a replay of the same post but not a later post with the same text), `MediaExportBoundTest` (1: `maxId` is 0 on an empty table and the bounded export page excludes a blob committed after the snapshot), `SourcePolicyTransactionTest` (6: a disable writes the flag and its gap together; setting a flag to the value it already holds writes nothing; a flag change whose gap write fails is not committed either; removing a source closes its gap in the same transaction; forgetting a source keeps the interval and drops the name; retention expires closed gaps and keeps the ones still happening; removing a source and its data takes the name off its gaps in one go and leaves another source's alone — the coordinator's own tests run against a mocked repository and can only show the hand-off, so what it is worth is decided here), `JournalLossTransactionTest` (3: accepting an event writes its loss with it; re-delivering the same event does not record it twice; an event whose loss cannot be written is not accepted either — the same reason, for the guard and the transaction the coordinator's journal fake was standing in for) — 27 tests | `./gradlew :platform:storage:connectedDebugAndroidTest` |
