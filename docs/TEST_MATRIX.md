...
| Search / conversation ViewModels | Locked and opening vault with mocked repositories | `SearchViewModelTest` (4: locked is reported as locked and the query is not run; a query typed while opening runs once ready and again after a retry; a full page keeps its cursor so the header can never call it a total; a page that verifies nothing ends the run instead of leaving a Load-more button that cannot help), `ConversationViewModelTest` (1: opening keeps loading, locked is shown as locked, ready shows the rows) | `./gradlew :feature:search:testDebugUnitTest :feature:conversation:testDebugUnitTest` |
