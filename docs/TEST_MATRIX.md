...
| Capture coordinator | Commit fence, cold start and maintenance with mocked repositories | `CaptureCoordinatorTest` (42 tests: switching a source off opens a gap that names it and switching it back on closes it, pausing one source never closes another's gap, messages dropped before parsing are a gap even though the ingest that followed succeeded, and its negative control — a message whose text was merely shortened is not a gap; that loss is written in the transaction that accepts the event, so an event the journal already holds does not record it a second time and replaying a pending row does not record it again; setting a
