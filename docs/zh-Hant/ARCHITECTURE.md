...
以及停用或移除來源後隨之而來的丟棄——都會先結清它。另外兩個出口不結清**這個**損失：解不出來的 payload 沒有東西可
結清；commit 重試額度用盡的列則記錄它自己的損失——整個事件（`COMMIT_FAILED`）——寫在把它標為 `FAILED`、清空 payload
的同一個 transaction 內：該列離開 `PENDING` 時一定帶著它的紀錄，否則整筆回滾（issue #28）。`event_journal.lossRecorded`
...
這個欄位是兩個位元——已結清、已延後——因為金庫拒絕寫入的結清既不是「已結清」也不只是「還沒結清」，也因為被放棄的列
不論它到達時的損失是否已結清都需要同一個停車位：恢復時把列還原成它原本的結清狀態，已消耗的認領在回程時仍然是已消耗。把這次失敗算進事件的 commit
額度，三次之後 payload 就被清空；把它留在 replay 的候選集合裡，它就佔住每一頁的開頭，數量夠多時後面的列全部
餓死。被拒絕的結清因此進入**延後**狀態：仍是 `PENDING`，payload 與認領都沒有動過，暫時離開兩個讀取者，直到某一輪
pass 把它放回去。replay 是在把其他所有列都排空之後才放回——放在 pass 的開頭，會讓一段持續失敗的前綴在每次觸發
時被重新插到所有列前面，後面的列永遠讀不到；結清走訪則只放回它自己那個來源的列，因為它跑在該來源的政策交易
內。重試由「證據」觸發而不是由計時器：一個自身帶著損失而被接受的事件，剛剛才在它的接受交易裡寫成一個缺口，
代表先前拒絕的那張表又收寫入了；沒有寫出缺口的接受不會觸發任何東西。

認領會回答它找到的是四種情況中的哪一種——它寫了缺口、缺口本來就在、該列已延後、該列已離開 `PENDING`——因為
只有前兩種代表損失已經在磁碟上。用 Boolean 會把第二與第三種混為一談，而一個手上拿著「另一趟 pass 延後該列之前
讀到的頁」的 replay，就會把它 commit 並清掉證據。pass 之間會合流也是同一個理由：頁面讀取在 pipeline 鎖之外，
兩趟重疊的 pass 正是「過期頁」得以存在的原因。請求設一個只有持閘門的 pass 才會清掉的旗標，而每個呼叫者都會**等**閘門
（持有者在 pass 中途被取消時會在離開時釋放閘門，下一個等待者會發現請求還在），遇到維護進行中的請求會保留給維護結束，
而不是被清掉。放棄紀錄寫不進去的列由同一套機制停放，下一輪 pass 恢復時再 commit 一次；殘餘與延後的結清相同——
停放列的裝置等待一次成功的缺口寫入或生命週期觸發，payload 完整——外加一條窄路：恢復剛好落在 pass 的最後一輪時，
那批列被放回卻沒有被嘗試，之後只能等生命週期觸發，而不是等證據。
