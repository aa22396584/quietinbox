...
- 有三個已知、尚未關閉的損失，都刻意留到各自的 commit：每一個都新增寫入路徑，而第 29 到 36 輪每一輪都在「修正 commit」裡找到新缺陷。**(a)** WhatsApp 群組內文剛好在換行處被截斷時會整列消失，而最後倖存的那一列可證明是完整的，因此既不會被標記、也不會記錄缺口；需要在 `ParsedBatch` 帶一個損失訊號，並寫在 commit 交易內（第 35 輪 Codex I1）。**(b)** 備份合併跳過重複列時，會連同該列帶的截短證據一起丟掉，因此把備份還原進已經有該訊息的金庫可能會失去那個標籤（第 35 輪 Codex I2）。**(c)** 結清失敗會讓整個來源 policy 交易回滾——這是對的，因為隨後的丟棄會清掉它最後一個讀得到的 payload——但 `HealthViewModel` 用 `runCatching {}` 丟掉結果，所以「停止擷取這個 app」會靜默失敗，來源仍然啟用而且什麼都沒說（第 36 輪 subagent I2）。這需要一個 ViewModel 會呈現的結果，以及五份字串目錄。
- commit 重試額度用盡的列會被標為 `FAILED`、payload 清空、完全沒有缺口，因此持續性的錯誤確實會遺失一個已接受的事件而且什麼都不說。這比 0.1.4 更早存在，記錄為 issue #28；0.1.4 只改變了一件事：**結清**失敗不再是把列推進那個洞的原因。
