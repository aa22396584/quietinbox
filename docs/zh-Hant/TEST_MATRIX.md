...
| L1 JVM 重播 | 純 Kotlin 的 parser／identity／reconcile／analytics | `core:*` 80 個測試（model 6、parser 13、identity 5、reconcile 22（含 `ReconcilerIdAlignmentTest`）、analytics 34）、`parsers:apps` 47 個（群組內文剛好在換行處被截斷時，批次會說有一整列遺失；截在最後一列內部時只有該列自己的標籤、批次不多說；沒被截的群組內文與 adapter 不拆列的內文都絕不宣稱遺失列）、`app` 5 個（提醒，含 `ReminderPolicy`）；兩個 1,000 次迭代的性質測試（property test）：不同的內容必須恰好被接受一次（seed 20260905）、沒有 id 的重複內容絕不可重複，且重播絕不可縮小視窗（seed 20260906） | `./gradlew :core:model:test :core:parser:test :core:identity:test :core:reconcile:test :core:analytics:test :parsers:apps:test` |
...
| 真機測試（instrumented）儲存 | 真實的 SQLCipher + Room 遷移 | `VaultRoundTripTest`（日誌 → commit → 搜尋 → 抑制 → 以持久化的金鑰重新開啟；刪除的會話在重播後不會復活；文字完全相同、但這次知道自己被截短的 repost，會把原本宣稱完整的那一列標記起來）、`MigrationTest`（對照匯出的 schema 執行 1→2、2→3 與 3→4；3→4 斷言兩個可為 null 的新欄位在既有列上都回傳 null、待處理的 journal 列保有 payload 與狀態且回傳「尚未記錄損失」——那正是它的事實，因為 0.1.3 從未記錄這種損失——並且之後可以插入帶來源的缺口與已結清的 journal 列）、`DemoDataTest`（示範資料寫入 → 筆數與投影 → 重複寫入不重複 → 清除後不留痕跡；App 語言為 zh-Hans／ja／ko 時，寫入內容經 `DemoLocalisation` 在地化；收件匣 lastActivity 是可見副本的 max observedAt，不是 sortKey）、`DeletionGraphTest`（11 個：journal 離開 PENDING 即清空 payload；刪除最新訊息後重算投影；到期副本在 retention 跑之前就隱藏、跑之後投影重算；移除來源並刪資料後不留任何東西；刪除全部經驗證且沒有快取的 cipher 沿用舊金鑰；空對話清理會留下尚未到期的 AMBIGUOUS_REPEAT、以及 empty-id 掃描後才 commit 的副本，真正全空的舊對話仍會刪除；收件匣與對話標題的預覽與筆數在新鮮查詢、時鐘跨過到期且沒有寫入、以及離開畫面再回來時，都不再顯示已到期副本）、`SearchPagingTest`（2 個：250 筆假陽性候選既藏不住真命中也不會讓頁面不足額，游標續頁不重疊；刪除 token 會抑制同一 post 的重播、但不抑制之後同文字的新 post）、`MediaExportBoundTest`（1 個：空表的 `maxId` 為 0，帶上限的匯出分頁排除快照之後才寫入的 blob）、`SourcePolicyTransactionTest`（13 個:停用會把旗標與缺口一起寫入;把旗標設成它已經是的值不寫入任何東西;缺口寫入失敗時旗標也不會被提交;移除來源會在同一個 transaction 內關閉它的缺口;「移除並刪除資料」保留區間、只抹去名稱,且不影響其他來源;retention 只清除已結束的缺口,仍在進行中的保留下來;停用來源會把它待處理列帶進來的損失一併結清、再把那些列丟棄,結清失敗時該列維持 PENDING、認領未被消耗、來源也仍是開啟,兩種 remove 分支中途失敗時來源、待處理列與缺口都原封不動,其中 `deleteData = false` 那條在拋錯前先寫下可觀察的東西;而一次已經走到七道圖形刪除中最後一道、才被 trigger 中止的移除,會把每一列都放回去——會話、訊息、媒體列、checkpoint、抑制、摘要與診斷——而且完全不動媒體檔案,因為檔案只從已提交交易回傳的清單裡刪除（協調器自己的測試跑在 mock 的 repository 上、只能證明它把工作交出去,真正的價值在這裡判定）、`JournalLossTransactionTest`（32 個:接受事件時把它帶來的損失一起寫入;同一 eventId 再次投遞不會二次記錄;損失寫不進去時該事件也不被接受;舊版帶進來的損失無論被幾道流程詢問都只由一次認領記錄;缺口寫不進去的認領不會被消耗;本版已接受的事件早已結清、無法再被認領;已離開 PENDING 的列完全無法認領;一再失敗的結清絕不消耗事件的 commit 額度;以及 commit 額度真的用盡時該列落入的那個洞的形狀（issue #28）;結清走訪在六種分頁大小下（含一種剛好等於列數）都恰好走訪該來源每一列一次且順序相同,其中三列共用同一毫秒,讓 eventId 這個決勝欄真的承重,整頁都解不出來時走訪仍會前進;它的查詢計畫是對複合索引的 seek、沒有暫時 B-tree;寫不進去的結清會讓該列進入延後狀態,payload、認領與 commit 額度都完好,離開 replay 的頁面讓後面的列被走到,下一輪 pass 恢復時再回來,而來源被丟棄前的走訪仍然看得到它；認領會回報它找到的是四種情況中的哪一種，其中只有兩種代表缺口已在磁碟上；已結清的列無法被退回延後狀態；整頁都解不出來的頁面是空的、而且仍然指向下一頁；查詢計畫是從 DAO 自己執行的那個 statement 解釋出來的，並且斷言游標是索引範圍的一部分、而不是套在其後的過濾；恢復某一個來源的延後列不會動到另一個來源的；parser 證明的損失會寫在儲存該批次的 commit 內，也寫在什麼都不儲存的那個 commit 出口上，而紀錄寫不進去的 commit 什麼都不儲存、列維持 PENDING；以及 issue #28 在真實金庫上：被放棄的列其紀錄與標記一起寫入且只寫一次；被拒絕的紀錄讓列維持 PENDING、額度與 payload 不動、停放在頁面之外、恢復後回到頁面；已結清的列停放再恢復後仍是已結清、認領仍是已消耗——接受路徑與舊版帶入路徑各測一次；四種候選讀取對四個值的看法一致，同一組資料在非空排除清單下只回傳未排除來源的 0/1；缺口已落地但標記被拒絕時會一起回滾；停放後恢復並成功 commit 的列什麼都沒丟；在認領之前就被停放的列回答「已延後」且不寫缺口；連停放都被金庫拒絕時列留在原地；已離開 PENDING 的列既不計費也不記錄）——共 69 個 | `./gradlew :platform:storage:connectedDebugAndroidTest` |
| 真機測試（instrumented）備份 | 真實金庫上的匯出 → 清空 → 匯入；維護閘門 | `BackupRoundTripTest`（4 個：備份只含看得見的副本、回報讀不到的媒體、還原後投影重算且媒體檔以目前金鑰可解密，且匯出時略過 blob、訊息仍為 LOCAL_COPY 的還原會把 `skippedMedia` 計入，設定頁才能警告；exclusive 進行中的匯出會被拒絕；不論先還原哪一份備份，合併都會補上既有列缺少的截短標籤；第二次完整匯入會把媒體接到既有 FAILED 列、不複製列、不動健康附件、也不覆蓋 DISABLED_BY_USER） | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 還原中斷 | 在每個出口被切斷的還原，只留下其資料列指向的檔案 | `BackupCancellationTest`（3 個：提交後的取消回傳已落地的還原、保留已連結的檔、移除未使用的 blob，第二次匯入不重複列；位元組解不出來的 blob 讓其訊息標為 `FAILED` 並計入結果；交易內的取消會回滾資料列並移除每一個檔）。負對照：把修剪移出交易使隨後的 `check` 失敗；拿掉 `catch` 的清理 | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 匯入卡住 | 不合作的備份 `read` 不得佔住 exclusive 維護 | `BackupHangTest`（21 個：取消永不返回、且 `close` 與 `read` 共用同一把鎖的串流，呼叫端仍會解除，該讀取仍卡住時第二次 exclusive 仍進得去；staging 之後銷毀金鑰世代則拒絕套用；open 與 apply 不在 main 上跑；兩條卡住的串流之後再重試不會再排入被堵住的 close；永不返回的匯出寫入不佔住 exclusive；取消其中一條卡住的匯入不會關掉另一條串流；staging 之後與 apply 之中 abort 都不會讓寫入落地、並回報金庫沒有變更；提交後 Job.cancel 仍回報還原完成；永不返回的匯入／匯出 `open` 不佔住 exclusive；Stop 之後才開啟的目的地不會被寫入；Stop 之後才開啟的匯入不會套用；只有 Job.cancel、沒有 abort 時遲到的目的地也不會被寫入；只有 Job.cancel、沒有 abort 時遲到的匯入也不會套用；close 拋錯是 Failed(IO) 不是 Ok；卡住的 close 不報 Ok 也不佔住 exclusive；取消匯出 B 不會拿到匯出 A 的 Ok，且 B 遲到開啟的串流寫入為 0；兩個已放棄但仍卡住的 close 佔住 write slot 上限；第二次立即返回的 close 不會在第一次 close 結束前釋放 write slot；加密暫存檔完成後取消不留下 staging 檔）。不是在 exclusive 外包一層 `withTimeout` | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 Settings 備份停止 | 透過設定頁的 abortBackup 在提交前後停止 | `SettingsBackupAbortTest`（4 個：提交後 Stop 顯示 Ok 且有一列，不是金庫未變更；套用前 Stop 顯示 ABORTED 且零列；SQL COMMIT 之後、Room 尚未返回交易時 Stop 仍顯示 Ok；加密暫存檔完成後 Stop 不留下 `backup-*.qibk`） | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 還原拒絕 | 被拒絕的還原讓金庫逐位元組維持原樣 | `BackupRejectionTest`（7 個：錯的復原金鑰、翻轉一個密文位元組、正文被截斷的檔、標頭內被截斷的檔、根本不是備份的檔、沒有空間放媒體的金庫；每個會碰到的資料表的每一欄、以及每個媒體檔，在前後比對）。負對照：`BackupRoundTripTest` 的正常路徑 | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 媒體複製與刪除競態 | 在連結交易前訊息已被刪的複製，不留下任何東西 | `MediaDeletionRaceTest`（2 個：在 copier 讀列與其交易之間刪掉訊息，不留 blob 列、孤兒或檔案；同一份複製在訊息仍在時會連結 blob 並保留其檔）。負對照：拿掉 `MediaCopier.store` 的 `!= 1` 檢查 | `./gradlew :platform:media:connectedDebugAndroidTest` |
...
| 擷取頁面 | 協調器拒絕的來源變更會到達使用者 | `HealthViewModelTest`（8 個，JVM：結清回滾會被呈現、以 App 名稱命名、並分類為結清；被拒絕的移除亦同；被拒絕的暫停是回滾而非結清；已提交但讀不回來源清單的變更不是回滾；鎖定的金庫是鎖定情況；未分類的例外不會變成回滾；成功的變更什麼都不說、已關閉的失敗保持關閉；取消絕不會被當成拒絕回報）、`PolicyFailureDialogTest`（6 個，實機，字串從資源讀出：結清、回滾、未分類、已提交未重載與鎖定各自只顯示自己的內文，未分類與已提交未重載不使用回滾文案，OK 會關閉） | `./gradlew :feature:health:testDebugUnitTest` · `./gradlew :feature:health:connectedDebugAndroidTest` |
...
| 擷取協調器 | 以 mock repository 驗證 commit 圍籬、冷啟動與維護 | `CaptureCoordinatorTest`（77 個測試：鎖定缺口寫入尚未返回時到達的 Ready 不會丟掉它；加上原本的 76 個：結清拒絕寫入是結清拒絕、已提交但重載失敗不是回滾、鎖定的金庫不會被包成拒絕變更；加上原本的 73 個：解析前就被丟棄的訊息會記為缺口，即使緊接著的 ingest 成功提交了倖存的訊息、InboxStyle 放不下的行是缺口而非內文被截短、其負向控制是「只被截短文字的訊息不算缺口」；該筆遺失寫在接受事件的同一個 transaction 內，因此 journal already 持有的事件不會二次記錄、重播待處理列也不會再記一次；0.1.3 以前留下的待處理列,其被丟棄的行無論重播幾次都只結清一次,同一批舊旗標若只可能代表整則訊息被丟棄也會結清,若同樣可能只是某則被截短則不動它,隨來源一起被丟棄的列則在 payload 還讀得到時結清；journal 與其後備缺口都寫不進去的損失會被保留,於下一件證明金庫可寫的事情發生時寫出——policy 載入,或僅僅是下一個被接受的事件（完全沒有 policy 變更）；把來源旗標設成它已經是的值不會產生第二個缺口、移除來源會關閉它留下的未結束缺口並在同時刪除資料時抹去其名稱、舊版留下的與自身政策矛盾的未結束缺口會在政策載入時被關閉、暫停後再關閉又開啟的來源仍保有「暫停中」缺口(在已關閉狀態下被暫停者亦然)；暫停後丟棄排隊事件、恢復時輪換 generation 與 session、來源清單載入後丟棄非來源套件、取消會傳播；等鎖期間被停用的來源事件永不寫入 journal、接受與 commit 之間的暫停讓事件維持 PENDING、暫停時不重播且恢復後重播、重播會丟棄來源已停用的列、維護執行會丟棄佇列並記錄精確缺口、每次維護都記錄自己的缺口；policy 未知前通知被原封不動地保留、只有來源會被建立 snapshot，金庫打不開時保留的通知被原封丟棄並記錄有界缺口；還在 copier 手上的 bitmap 仍計入佇列上限；保留緩衝溢位會記錄丟棄並只留下來源；journal 寫入拋例外會記為缺口；金庫鎖定期間無法記錄的冷啟動遺失與管線鎖定會在金庫開啟後補記為有界缺口；補記失敗的遺失會保留到下一次 policy 載入再寫、policy 載入後才落地的缺口列會立刻關閉、跨越斷線仍被保留的通知會得到自己的缺口，但來源已暫停或 resync 已再次擷取時不記；落在補記與旗標翻轉之間的缺口列由 policy 載入關閉；寫入失敗的溢位缺口會保留到下一次 policy 載入再寫；同 key 但 post 時間較舊的過期副本是獨立的遺失；釋放途中的斷線會給較晚的通知自己的缺口、而不是讓它被自己抑制；從未啟用的 app 只會被讀取套件名稱；金庫拒絕寫入的結清會讓該列進入延後狀態、不再佔住頁面，兩百列這樣的列不再餓死第二百零一列，同一輪裡第二個來源的列也不會被餓死，而且完全不消耗任何 commit 額度；延後的列由下一次「寫出了缺口的接受」重試，反向控制是一次沒有寫出缺口的接受；被停用或移除丟棄之前，延後的列仍然會先結清；隨來源一起移除的待處理列，也會趁 payload 還讀得到時先結清——`removeSource` 先前完全沒有這條測試；沒有取得任何東西的認領會說明它找到的是四種情況中的哪一種，因此被另一趟 pass 延後的列，不會再因為一個「本來代表缺口已寫入」的 `false` 而被 commit；replay 之間會合流，同時到達的兩個觸發只會跑一趟，不會有兩趟 pass 同時持有重疊的頁；比一整趟 pass 還長的失敗前綴，不再讓後面的列永遠讀不到（頁大小與輪數上限由測試設定）；連延後都寫不進去的 replay 不會宣稱自己有進展；超過一頁的來源，其待處理列會在停用前全部結清；parser 證明被整列切掉的那一列會以缺口交給 commit，切在最後一列內部的情況是它的對照；commit 額度用盡的列會被放棄並把整個事件的損失記錄一次，紀錄被拒絕時它會被停放而後面的列仍被走到，既記不了也停不了的列不算進展，現場入口交出的紀錄與重播入口相同；正在跑的 pass 被取消時抵達的請求仍會被服務，遇到維護進行中的請求會保留給維護結束，關掉一個來源只恢復那個來源的延後列，一次延後只武裝一次重試、不論之後有幾次缺口寫入成功） | `./gradlew :platform:capture:testDebugUnitTest` |
...
| 搜尋／對話 ViewModel | 以 mock repository 驗證鎖定與開啟中的金庫 | `SearchViewModelTest`（13 個：鎖定時顯示鎖定且不執行查詢；開啟中輸入的查詢在就緒後執行、重試解鎖後再執行一次；整頁塞滿時保留 cursor，所以標頭永遠不會把它講成總數；一頁一個都驗不出來時仍保留 cursor，因為空頁代表候選掃描預算用盡、不是索引已到底；查詢變更時仍在飛行中的那一頁會被丟棄，不會貼到別人的結果後面；新查詢絕不會帶上一次搜尋的 cursor（mixed-cursor 表）；過期的 load-more 不得解開較新頁的 in-flight 旗標（stale-unlock 表）；A→B→A 只套用後面那次 A 的頁；debounce 期間切來源會作廢 cursor；第一頁與下一頁亂序完成時仍是同一 session 的前綴；repository 丟出例外是失敗搜尋可重試，不是空成功；load-more 失敗保留已顯示的命中與 cursor；CancellationException 不會變成空成功）、`ConversationViewModelTest`（1 個：開啟中維持載入、鎖定顯示鎖定、就緒顯示列） | `./gradlew :feature:search:testDebugUnitTest :feature:conversation:testDebugUnitTest` |
| Onboarding 判定 | 擷取測試可以被稱為成功的條件，使用 mock 的 repository | `OnboardingViewModelTest`（6 項：金庫裡本來就有訊息不會讓測試通過；三則中擷取到一則不算成功；三則全到才算成功且絕不同時被判為失敗；副本一直沒到會被回報為失敗而不是一直轉；副本晚到算成功，不是逾時失敗；重試會取消先前的等待，因此第一個計時器不會把第二次送出判成失敗） | `./gradlew :feature:onboarding:testDebugUnitTest` |
