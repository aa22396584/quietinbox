...
| `T016` | 群組與堆疊通知：由另一個知情同意的測試帳號在合成群組中傳送 `T016 alpha`、`T016 beta`（只有來源 App 要求時，才加入其他知情同意的合成測試帳號成員），使來源貼出群組通知，並在支援時形成堆疊／摘要通知。 | 存下每一個通知可見列，不得捏造隱藏列；來源省略的發送者／聊天室身分或列維持 `UNKNOWN`，可證明遺失時誠實標示缺口／截短；結果為 `UNVERIFIED`。 | 開啟群組／堆疊副本不得標記任何來源聊天室已讀、清除通知、回覆或以其他方式操作來源。 |
