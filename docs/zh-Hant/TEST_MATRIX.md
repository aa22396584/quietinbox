...
| 真機測試（instrumented）備份 | 真實金庫上的匯出 → 清空 → 匯入；維護閘門 | `BackupRoundTripTest`（3 個：備份只含看得見的副本、回報讀不到的媒體、還原後投影重算且媒體檔以目前金鑰可解密；exclusive 進行中的匯出會被拒絕；不論先還原哪一份備份，合併都會補上既有列缺少的截短標籤） | `./gradlew :platform:backup:connectedDebugAndroidTest` |
...
| 真機測試 還原拒絕 | 被拒絕的還原讓金庫逐位元組維持原樣 | `BackupRejectionTest`（6 個：錯的復原金鑰、翻轉一個密文位元組、正文被截斷的檔、標頭內被截斷的檔、根本不是備份的檔、沒有空間放媒體的金庫；每個會碰到的資料表的每一欄、以及每個媒體檔，在前後比對）。負對照：`BackupRoundTripTest` 的正常路徑 | `./gradlew :platform:backup:connectedDebugAndroidTest` |
