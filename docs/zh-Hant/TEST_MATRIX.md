...
| 搜尋／對話 ViewModel | 以 mock repository 驗證鎖定與開啟中的金庫 | `SearchViewModelTest`（4 個：鎖定時顯示鎖定且不執行查詢；開啟中輸入的查詢在就緒後執行、重試解鎖後再執行一次；整頁塞滿時保留 cursor，所以標頭永遠不會把它講成總數；一頁一個都驗不出來時就結束，不留下一顆按了也沒用的「載入更多」）、`ConversationViewModelTest`（1 個：開啟中維持載入、鎖定顯示鎖定、就緒顯示列） | `./gradlew :feature:search:testDebugUnitTest :feature:conversation:testDebugUnitTest` |
