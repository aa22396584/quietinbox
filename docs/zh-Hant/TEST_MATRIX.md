...
## 真實來源相容性流程（Issue #22，RS-02）

以下是未來 L3 查核的流程定義，不是執行證據，也不是已經跑過的測試。只能使用兩個知情同意的測試帳號與
合成標記文字。在 `COMPATIBILITY.md` 尚未加入該來源精確版本與情境的證據紀錄前，結果一律是 `UNVERIFIED`。

| ID | 情境與流程 | 預期結果（含不確定性） | 唯讀來源檢查 |
| --- | --- | --- | --- |
| `T001` | 私訊：開啟預覽，由第二個帳號傳送 `T001 alpha`，只在靜讀開啟存下的副本。 | 靜讀應只存一次通知可見的發送者與標記；來源未提供的身分信心與欄位維持 `UNKNOWN`；留下證據紀錄前為 `UNVERIFIED`。 | 開啟副本不得標記來源聊天室已讀、清除其通知、回覆或以其他方式操作來源。 |
| `T004` | 貼圖／媒體：傳送以歷史標記 `T004 sticker` 識別的貼圖；若來源支援，再傳一張合成圖片，分別記錄通知列實際暴露的內容。 | 保留通知可見文字，或在可取得時保留佔位／本機副本；媒體位元組、說明與類型可能缺少或為 `UNKNOWN`；留下證據紀錄前為 `UNVERIFIED`。 | 開啟任一副本不得標記來源聊天室已讀、清除通知、透過來源 App 下載媒體或以其他方式操作來源。 |
| `T016` | 群組與堆疊通知：由另一個知情同意的測試帳號在合成群組中傳送 `T016 alpha`、`T016 beta`（只有來源 App 要求時，才加入其他知情同意的合成測試帳號成員），使來源貼出群組通知。不要開啟或清除該通知，再於第二個不同的合成對話（私訊或另一個合成群組）傳送 `T016 gamma`，讓兩個不同的合成對話通知同時保持作用中，使來源在支援時形成堆疊／摘要通知。 | 存下每一個通知可見列，不得捏造隱藏列；來源省略的發送者／聊天室身分或列維持 `UNKNOWN`，可證明遺失時誠實標示缺口／截短；結果為 `UNVERIFIED`。 | 開啟群組／堆疊副本不得標記任何來源聊天室已讀、清除通知、回覆或以其他方式操作來源。 |
| `T017` | 相同文字重複傳送：把 `T017 same` 當成兩則不同私訊傳送，並保留兩個來源 post 足夠久以觀察通知更新。 | 來源有證據證明兩則不同時保留兩則；否則保留 `AMBIGUOUS_REPEAT`／`UNKNOWN` 結果，不武斷判為重複或合併；結果為 `UNVERIFIED`。 | 開啟任一副本不得標記來源聊天室已讀、清除任一通知、回覆或以其他方式操作來源。 |
| `T045` | 關閉預覽：在來源 App 或 Android 關閉訊息預覽，傳送 `T045 preview off`，只記錄 listener 實際看見的佔位內容。 | 只存通知暴露的內容並標示疑似預覽受限；遮蔽由 Android 或來源 App 造成仍為 `UNKNOWN`；結果為 `UNVERIFIED`。 | 開啟受限副本不得標記來源聊天室已讀、揭露／抓取隱藏文字、清除通知或以其他方式操作來源。 |
