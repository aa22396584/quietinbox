...
| L1 JVM 重播 | 純 Kotlin 的 parser／identity／reconcile／analytics | `core:*` 79 個測試（model 5、parser 13、identity 5、reconcile 22（含 `ReconcilerIdAlignmentTest`）、analytics 34）、`parsers:apps` 47 個（群組內文剛好在換行處被截斷時，批次會說有一整列遺失；截在最後一列內部時只有該列自己的標籤、批次不多說；沒被截的群組內文與 adapter 不拆列的內文都絕不宣稱遺失列）、`app` 5 個（提醒，含 `ReminderPolicy`）；兩個 1,000 次迭代的性質測試（property test）：不同的內容必須恰好被接受一次（seed 20260905）、沒有 id 的重複內容絕不可重複，且重播絕不可縮小視窗（seed 20260906） | `./gradlew :core:model:test :core:parser:test :core:identity:test :core:reconcile:test :core:analytics:test :parsers:apps:test` |
...
| 真機測試 還原中斷 | 在每個出口被切斷的還原，只留下其資料列指向的檔案 | `BackupCancellationTest`（3 個：提交後才落地的取消會保留已連結的檔、移除其餘，做兩次，第二次匯入是重複資料；金庫寫不進的 blob 讓其訊息標為 `FAILED` 並計入結果；交易內的取消會回滾資料列並移除每一個檔）。負對照：把修剪移回 `withTransaction` 之後、拿掉 `catch` 的清理 | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 還原拒絕 | 被拒絕的還原讓金庫逐位元組維持原樣 | `BackupRejectionTest`（6 個：錯的復原金鑰、翻轉一個密文位元組、正文被截斷的檔、標頭內被截斷的檔、根本不是備份的檔、沒有空間放媒體的金庫；每個資料列數與每個媒體檔在前後比對）。負對照：`BackupRoundTripTest` 的正常路徑 | `./gradlew :platform:backup:connectedDebugAndroidTest` |
