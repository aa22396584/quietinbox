...
| 擷取頁面 | 協調器拒絕的來源變更會到達使用者 | `HealthViewModelTest`（5 個，JVM：被拒絕的關閉會被呈現、以 App 名稱命名、並歸因於結清；被拒絕的移除亦同；被拒絕的暫停不帶那段結清說明；成功的變更什麼都不說、已關閉的失敗保持關閉；取消絕不會被當成拒絕回報）、`PolicyFailureDialogTest`（3 個，實機，字串從資源讀出：每種內文只在它的情況出現，OK 會關閉） | `./gradlew :feature:health:testDebugUnitTest` · `./gradlew :feature:health:connectedDebugAndroidTest` |
