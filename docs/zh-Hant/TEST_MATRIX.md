...
| 真機測試 匯入卡住 | 不合作的備份 `read` 不得佔住 exclusive 維護 | `BackupHangTest`（3 個：取消永不返回、且 `close` 與 `read` 共用同一把鎖的串流，呼叫端仍會解除，該讀取仍卡住時第二次 exclusive 仍進得去；staging 之後銷毀金鑰世代則拒絕套用；open 與 apply 不在 main 上跑）。不是在 exclusive 外包一層 `withTimeout` | `./gradlew :platform:backup:connectedDebugAndroidTest` |
