...
| 真機測試 匯入卡住 | 不合作的備份 `read` 不得佔住 exclusive 維護 | `BackupHangTest`（15 個：取消永不返回、且 `close` 與 `read` 共用同一把鎖的串流，呼叫端仍會解除，該讀取仍卡住時第二次 exclusive 仍進得去；staging 之後銷毀金鑰世代則拒絕套用；open 與 apply 不在 main 上跑；兩條卡住的串流之後再重試不會再排入被堵住的 close；永不返回的匯出寫入不佔住 exclusive；取消其中一條卡住的匯入不會關掉另一條串流；staging 之後與 apply 之中 abort 都不會讓寫入落地、並回報金庫沒有變更；提交後 Job.cancel 仍回報還原完成；永不返回的匯入／匯出 `open` 不佔住 exclusive；Stop 之後才開啟的目的地不會被寫入；Stop 之後才開啟的匯入不會套用；close 拋錯是 Failed(IO) 不是 Ok；卡住的 close 不報 Ok 也不佔住 exclusive）。不是在 exclusive 外包一層 `withTimeout` | `./gradlew :platform:backup:connectedDebugAndroidTest` |
| 真機測試 Settings 備份停止 | 透過設定頁的 abortBackup 在提交前後停止 | `SettingsBackupAbortTest`（2 個：提交後 Stop 顯示 Ok 且有一列，不是金庫未變更；套用前 Stop 顯示 ABORTED 且零列） | `./gradlew :platform:backup:connectedDebugAndroidTest` |
