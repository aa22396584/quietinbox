...
| 真機測試 匯入卡住 | 不合作的備份 `read` 不得佔住 exclusive 維護 | `BackupHangTest`（11 個：取消永不返回、且 `close` 與 `read` 共用同一把鎖的串流，呼叫端仍會解除，該讀取仍卡住時第二次 exclusive 仍進得去；staging 之後銷毀金鑰世代則拒絕套用；open 與 apply 不在 main 上跑；兩條卡住的串流之後再重試不會再排入被堵住的 close；永不返回的匯出寫入不佔住 exclusive；取消其中一條卡住的匯入不會關掉另一條串流；staging 之後與 apply 之中 abort 都不會讓寫入落地、並回報已停止且沒有變更；提交後 abort 仍回報還原完成；永不返回的匯入／匯出 `open` 不佔住 exclusive）。不是在 exclusive 外包一層 `withTimeout` | `./gradlew :platform:backup:connectedDebugAndroidTest` |
