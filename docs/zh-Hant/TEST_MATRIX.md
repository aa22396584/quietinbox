...
| 真機測試 匯入卡住 | 不合作的備份 `read` 不得佔住 exclusive 維護 | `BackupHangTest`（2 個：取消永不返回的 `read` 會讓呼叫端解除，且該讀取仍卡住時第二次 exclusive 仍進得去；staging 之後銷毀金鑰世代則拒絕套用）。不是在 exclusive 外包一層 `withTimeout` | `./gradlew :platform:backup:connectedDebugAndroidTest` |
